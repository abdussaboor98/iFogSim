...
\If{$f.type = $ SERVER\_CRASH}
    \State Remove $c$ from $sourceServer$ without checkpointing
    \State $c.resetProgress()$ \Comment{Work progress lost}
\Else \Comment{CPU or bandwidth degradation}
...
\State $localTarget \gets \text{SelectLocalServer}(c, S_i)$ \Comment{Best healthy server}
\If{$localTarget \neq \text{null}$}
    \State Place $c$ on $localTarget$
...
\If{cloud is available}
    ...
    \State Resume or restart $c$ on cloud resources
