...
The system is configured with the following parameters from \texttt{collabft-config.yaml}:

\begin{itemize}
    \item \textbf{Mode}: 1 (Decentralized with gossip)
...
\Require $\tau_{bw}$, $\tau_{mig}$, $\tau_{sla}$: Error thresholds
...
\begin{algorithmic}[1]
\Require $\lambda_{MTBF}$: Mean Time Between Failures, $T_{pred}$: Prediction lead time, $T_{recovery}$: Recovery duration
