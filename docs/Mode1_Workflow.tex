...
\begin{equation}
    \text{InterArrival} = -\bar{\tau} \cdot \ln(1 - U), \quad U \sim \text{Uniform}(0,1), \quad \bar{\tau} = 160 \text{ seconds}
\end{equation}
...
\begin{align}
    R_{cpu} &\sim \text{Uniform}[200, 600] \text{ MIPS} \\
    T_{exec} &\sim \text{Uniform}[200, 400] \text{ seconds} \\
    R_{mem} &\sim \text{Uniform}[128, 512] \text{ MB} \\
    R_{bw} &\sim \text{Uniform}[150, 550] \text{ Mbps} \\
    S_{container} &\sim \text{Uniform}[200, 420] \text{ MB}
\end{align}
...
\begin{equation}
    \text{TimeBetweenFaults} = -\text{MTBF} \cdot \ln(1 - U), \quad \text{MTBF} = 45 \text{ seconds}
\end{equation}
...
\begin{equation}
    \text{FaultType} = \begin{cases}
        \text{CPU FAILURE} & \text{if } U < 0.33 \text{ (CPU reduced to 20\%)} \\
        \text{BANDWIDTH DEGRADATION} & \text{if } 0.33 \leq U < 0.67 \text{ (BW reduced to 30\%)} \\
        \text{SERVER CRASH} & \text{if } 0.67 \leq U < 1.0 \text{ (complete failure)}
    \end{cases}
\end{equation}
...
\begin{enumerate}
    ...
    \begin{equation}
        \text{degradation} = \begin{cases}
            \text{cpuFactor} = 0.2 & \text{for CPU FAILURE} \\
            \text{bwFactor} = 0.3 & \text{for BANDWIDTH DEGRADATION} \\
            \text{failed} = \text{true} & \text{for SERVER CRASH}
        \end{cases}
    \end{equation}
...
\begin{enumerate}
    ...
    \item If trust enabled: filter out nodes with $\text{trust} < 0.5$
...
\begin{enumerate}
    ...
    \begin{equation}
        \text{Reject if } \text{trustEnabled} \land \text{trust}(\text{bidderId}) < 0.5
    \end{equation}
...
\begin{equation}
    \text{EffectiveBid} = \begin{cases}
        \frac{\text{BidValue}}{\max(10^{-6}, \text{trust}(\text{bidderId}))} & \text{if trustEnabled} \\
...
\begin{itemize}
    \item \textbf{Payment Amount} = BidValue (from winning bid)
    \item \textbf{Transfer}:
    \begin{itemize}
        \item Debit: Owner fog node
        \item Credit: Winner fog node
    \end{itemize}
\end{itemize}
...
\begin{equation}
    \text{trust}(i) = 1.0, \quad \forall i \in \text{FogNodes}
\end{equation}

Trust values are bounded:
\begin{equation}
    \text{trust}(i) \in [0, 1], \quad \forall i
\end{equation}
...
\State $t_{current} \gets \text{trust}(i)$
\State $t_{updated} \gets t_{current}$
\State
\If{$d_{bw}$ \textbf{or} $d_{mig}$} \Comment{Multiplicative decay for explicit lies}
    \State $t_{updated} \gets t_{current} \times \delta_{decay}$
\EndIf
\State
\If{$d_{sla}$} \Comment{Additive penalty for SLA violations}
    \State $t_{updated} \gets \max(0, t_{updated} - \beta_{violation})$
\ElsIf{$\neg d_{bw}$ \textbf{and} $\neg d_{mig}$} \Comment{Bonus only if no lies}
    \State $t_{updated} \gets \min(1, t_{updated} + \beta_{success})$
\EndIf
\State
\State $\text{trust}(i) \gets t_{updated}$
...
\begin{equation}
    t'(i) = \begin{cases}
        t(i) \times \delta_{decay} & \text{if } d_{bw} \lor d_{mig} \\
        t(i) & \text{otherwise}
    \end{cases}
\end{equation}
...
\begin{equation}
    t''(i) = \begin{cases}
        \max(0, t'(i) - \beta_{violation}) & \text{if } d_{sla} \\
        \min(1, t'(i) + \beta_{success}) & \text{if } \neg d_{bw} \land \neg d_{mig} \land \neg d_{sla} \\
        t'(i) & \text{otherwise}
    \end{cases}
\end{equation}
...
where:
\begin{align}
    \beta_{violation} &= 0.10 \text{ (10\% penalty for SLA violations)} \\
    \beta_{success} &= 0.02 \text{ (2\% reward for good performance)}
\end{align}
...
\begin{equation}
    \text{Eligible}(i) = \begin{cases}
        \text{true} & \text{if } \text{trust}(i) \geq \theta_{trust} \\
        \text{false} & \text{otherwise}
    \end{cases}
\end{equation}
...
\begin{equation}
    \text{EffectiveBid}(i) = \frac{\text{BidValue}(i)}{\max(10^{-6}, \text{trust}(i))}
\end{equation}

\textbf{Example}:
\begin{itemize}
    \item BidValue = 10.0, trust = 1.0 → EffectiveBid = 10.0
    \item BidValue = 10.0, trust = 0.5 → EffectiveBid = 20.0
    \item BidValue = 10.0, trust = 0.25 → EffectiveBid = 40.0
\end{itemize}

\subsection{Trust Recovery Mechanism}

\subsubsection{Recovery After Honest Behavior}
...
\begin{equation}
    \text{Winner} = \arg\min_{i \in \text{Feasible}} \left\{ \frac{\text{BidValue}(i)}{\text{trust}(i)} \right\}
\end{equation}
...
\State $t_{current} \gets \text{trust}(executor)$
\State $t_{updated} \gets t_{current}$
\State
\State \Comment{Phase 1: Multiplicative penalty for bandwidth/migration lies}
\If{$d_{bw}$ \textbf{or} $d_{mig}$}
    \State $t_{updated} \gets t_{current} \times \delta_{decay}$ \Comment{50\% decay}
\EndIf
\State
\State \Comment{Phase 2: Additive adjustment for SLA performance}
\If{$d_{sla}$}
    \State $t_{updated} \gets \max(0, t_{updated} - \beta_{violation})$ \Comment{-10\% penalty}
\ElsIf{$\neg d_{bw}$ \textbf{and} $\neg d_{mig}$ \textbf{and} $\neg d_{sla}$}
    \State $t_{updated} \gets \min(1, t_{updated} + \beta_{success})$ \Comment{+2\% reward}
\EndIf
\State
\State \Comment{Update trust table}
\State $\text{trust}(executor) \gets t_{updated}$
\State
\State \Comment{Log trust change}
\State $\text{recordTrustEvaluation}(container, executor, m_{sla}, t_{current}, t_{updated}, d_{sla})$
\State \Return $t_{updated}$
\end{algorithmic}
\end{algorithm}
...
\Require Minimum trust threshold $\theta_{trust} = 0.5$
...
\For{each $bid \in bids$}
    ...
    \State $\text{effectiveBid} \gets \frac{bid.\text{getValue}()}{\max(10^{-6}, t)}$
    \State $evaluation \gets \text{createEvaluation}(bid, \text{effectiveBid}, t)$
...
\begin{algorithmic}[1]
\Require Current trust value $t_{current}$
\Require Target trust threshold $\theta_{trust} = 0.5$
\Require Recovery increment per honest task $\beta_{success} = 0.02$
\Ensure Number of honest tasks required to reach threshold
\If{$t_{current} \geq \theta_{trust}$}
    \State \Return 0 \Comment{Already above threshold}
\EndIf
\State
\State $deficit \gets \theta_{trust} - t_{current}$
\State $tasksRequired \gets \lceil \frac{deficit}{\beta_{success}} \rceil$
\State
\State \Return $tasksRequired$
\end{algorithmic}
\end{algorithm}
